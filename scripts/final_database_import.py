#!/usr/bin/env python3
"""
Final Database Import for StreamWatch Data
Properly imports all data using correct column names
"""
...
def import_sites_data(conn):
    """Import sites data with correct column names"""
...
def import_biological_data(conn):
    """Import biological data with correct column names"""
    ...
    try:
        ...
        for _, row in df_bugs.iterrows():
            ...
            try:
                # Extract site from sample code (e.g., "AC5_2021-11-20" -> "AC5")
...
def test_database(conn):
    ...
    if bio_count > 0:
        ...
        df = pd.read_sql_query("SELECT taxon, SUM(count) as total_count FROM biological_data GROUP BY taxon ORDER BY total_count DESC LIMIT 10", conn)
        print(df.to_string(index=False))

def main():
    """Main function to import all data properly"""
    print("FINAL STREAMWATCH DATABASE IMPORT")
    print("=" * 50)
    ...
    try:
        ...
        print("Run 'python database_explorer.py' to explore the data.")
